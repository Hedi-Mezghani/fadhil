...
export class FileUploadContratService {
  private apiUrl = 'http://localhost:8089/api/contratupload';
...
uploadFiles(files: any[], userId: number): Observable<any> {
  ...
  return this.http.post<any>('http://localhost:8089/api/contratupload/upload', formData, { headers })
    .pipe(
      catchError((error) => {
